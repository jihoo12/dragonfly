{-# OPTIONS_GHC -Wno-unused-imports -Wno-unused-matches -Wno-name-shadowing -Wno-incomplete-uni-patterns -Wno-incomplete-patterns -Wno-x-partial -Wno-orphans #-}
-- Adapted from cubicaltt; see vendor/cubicaltt/LICENSE and README.md.
{-# LANGUAGE TypeSynonymInstances, FlexibleInstances #-}
module Dragonfly.Cubical.Eval where
...
instance Nominal Val where
  ...
  act u (i, phi) | i `notElem` support u = u
    ...
    in case u of
         ...
         VGlue l a ts              -> glue l (acti a) (acti ts)
         VGlueElem a ts          -> glueElem (acti a) (acti ts)
         VUnGlueElem a b ts        -> unglueElem (acti a) (acti b) (acti ts)
         VUnGlueElemU a b es     -> unGlueU (acti a) (acti b) (acti es)
         VCompU l a ts             -> compUniv l (acti a) (acti ts)
...
eval rho@(Env (_,_,_,Nameless os)) v = case v of
  ...
  Lam{}               -> Ter v rho
  Split{}             -> Ter v rho
  Sum{}               -> Ter v emptyEnv -- checked sums are closed and nullary
  HSum{}              -> Ter v rho
  Undef{}             -> Ter v rho
  Hole{}              -> Ter v rho
...
comp i a u ts = case a of
  ...
  VU l -> compUniv l u (Map.map (VPLam i) ts)
  VCompU _ a es | not (isNeutralU i es u ts)  -> compU i a es u ts
  VGlue _ b equivs | not (isNeutralGlue i equivs u ts) -> compGlue i b equivs u ts
...
glue :: Natural -> Val -> System Val -> Val
glue l b ts | eps `member` ts = equivDom (ts ! eps)
          | otherwise       = VGlue l b ts

glueElem :: Val -> System Val -> Val
glueElem v us | eps `member` us = us ! eps
glueElem v us = VGlueElem v us

unglueElem :: Val -> Val -> System Val -> Val
unglueElem w b isos | eps `member` isos = app (equivFun (isos ! eps)) w
                  | otherwise         = case w of
                                          VGlueElem v us -> v
...
compUniv :: Natural -> Val -> System Val -> Val
compUniv l b es | eps `Map.member` es = (es ! eps) @@ One
              | otherwise           = VCompU l b es
...
instance Convertible Val where
  conv ns u v | u == v    = True
    ...
    in case (u,v) of
      ...
      (VGlue l v equivs,VGlue l' v' equivs')   -> l == l' && conv ns (v,equivs) (v',equivs')
      (VGlueElem (VUnGlueElem b _ equivs) ts,g) -> conv ns (border b equivs,b) (ts,g)
      (g,VGlueElem (VUnGlueElem b _ equivs) ts) -> conv ns (border b equivs,b) (ts,g)
      ...
      (VCompU l u es,VCompU l' u' es')              -> l == l' && conv ns (u,es) (u',es')
...
instance Normal Val where
  normal ns v = case v of
    ...
    VGlue l u equivs      -> VGlue l (normal ns u) (normal ns equivs)
    VGlueElem u us      -> VGlueElem (normal ns u) (normal ns us)
    VUnGlueElem u b us    -> VUnGlueElem (normal ns u) (normal ns b) (normal ns us)
    VUnGlueElemU e u us -> VUnGlueElemU (normal ns e) (normal ns u) (normal ns us)
    VCompU l a ts         -> VCompU l (normal ns a) (normal ns ts)
