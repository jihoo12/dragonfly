...
module Dragonfly.Cubical.Syntax where
...
data Ter = Pi Ter
         ...
         -- Glue
         | Glue Natural Ter (System Ter)
         | GlueElem Ter Ter (System Ter) -- type, base, partial elements
         | UnGlueElem Ter Ter -- element and explicit Glue type
           -- Id
         | Id Ter Ter Ter
...
showTer :: Ter -> Doc
showTer v = case v of
  U l                -> text "Type" <+> integer (toInteger l)
  Ann t a            -> parens (showTer t <+> colon <+> showTer a)
  ...
  Glue l a ts        -> text ("Glue[" ++ show l ++ "]") <+> showTer1 a <+> text (showSystem ts)
  GlueElem _ a ts      -> text "glue" <+> showTer1 a <+> text (showSystem ts)
  UnGlueElem a ty    -> text "unglue" <+> showTer1 a <+> showTer1 ty
...
showVal :: Val -> Doc
showVal v = case v of
  VU l              -> text "Type" <+> integer (toInteger l)
  ...
  VGlue l a ts        -> text ("Glue[" ++ show l ++ "]") <+> showVal1 a <+> text (showSystem ts)
  VGlueElem a ts    -> text "glue" <+> showVal1 a <+> text (showSystem ts)
  VUnGlueElem a _ ts -> text "unglue" <+> showVal1 a <+> text (showSystem ts)
  VUnGlueElemU v b es -> text "unglue U" <+> showVals [v,b]
                         <+> text (showSystem es)
  VCompU l a ts       -> text ("comp (<_> Type " ++ show l ++ ")") <+> showVal1 a <+> text (showSystem ts)
